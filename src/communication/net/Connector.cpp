...
void Connector::connect() {
  int sockfd = ::socket(serverAddr_.family(),
                        SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0);
  if (sockfd < 0) {
    LOG_FATAL << "create socket failed!";
  }
  int ret =
      ::connect(sockfd, serverAddr_.getSockAddr(), serverAddr_.getSockLen());
...
void Connector::retry(int sockfd) {
  ...
  if (connect_) {
    ...
    TimerManager::getInstance()->addTimer(
        static_cast<unsigned int>(retryDelayMs_),
...
bool Connector::isSelfConnect(int sockfd) {
  ...
  if (localaddr.sun_family == AF_INET) {
    ...
    const struct sockaddr_in6 *raddr6 =
        reinterpret_cast<struct sockaddr_in6 *>(&peeraddr);
