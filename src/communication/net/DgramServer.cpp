...
DgramServer::DgramServer(EventLoop* loop, const InetAddress& listenAddr, const std::string& nameArg)
    : loop_(CHECK_NOTNULL(loop))
    , socket_(::socket(listenAddr.family(), SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0))
...
void DgramServer::send(const InetAddress& clientAddr, const void* message, int len)
{
    ssize_t ret = ::sendto(socket_.fd(), message, static_cast<unsigned long long>(len), 0, clientAddr.getSockAddr(),
                           clientAddr.getSockLen());
    if (ret < 0)
    {
...
void DgramServer::handleRead()
{
    ...
    ssize_t nr = ::recvfrom(socket_.fd(), message, sizeof(message), 0, reinterpret_cast<struct sockaddr*>(&peerAddr),
                            &addrLen);
