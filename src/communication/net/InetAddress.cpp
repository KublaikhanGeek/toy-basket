...
static_assert(sizeof(InetAddress) == (sizeof(struct sockaddr_un) + 2),
              "InetAddress is same size as sockaddr_un + 2");
...
bool InetAddress::resolve(const std::string &hostname, InetAddress *out) {
  ...
  if (ret == 0 && he != NULL) {
    if (he->h_addrtype == AF_INET) {
      ...
      out->addr_in6_.sin6_addr =
          *reinterpret_cast<struct in6_addr *>(he->h_addr);
