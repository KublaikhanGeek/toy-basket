...
bool Socket::getTcpInfoString(char *buf, int len) const {
  ...
  if (ok) {
    snprintf(
        buf, static_cast<unsigned long long>(len),
        "unrecovered=%u "
...
void Socket::bindAddress(const struct sockaddr *localaddr) {
  if (localaddr->sa_family == AF_UNIX) {
    const struct sockaddr_un *addrun =
        reinterpret_cast<const struct sockaddr_un *>(localaddr);
...
int Socket::accept(struct sockaddr *peeraddr) {
  ...
  int connfd =
      ::accept4(sockfd_, peeraddr, &addrlen, SOCK_NONBLOCK | SOCK_CLOEXEC);
  if (connfd < 0) {
    int savedErrno = errno;
    LOG_ERROR << "Socket::accept";
    switch (savedErrno) {
...
///
ssize_t Socket::sendto(const void *buf, size_t len, const char *path,
                       int sendto_flags) {
  if (buf == NULL) {
  ...
  if (strlen(path) > sizeof(saddr.sun_path) - 1) {
    LOG_ERROR
        << "sendto_unix_dgram_socket: UNIX destination socket path too long";
    return -1;
  }
  ...
  bytes = ::sendto(sockfd_, buf, len, sendto_flags,
...
ssize_t Socket::sendto(const void *buf, size_t length, const std::string &path,
                       int sendto_flags) {
  return sendto(buf, length, path.c_str(), sendto_flags);
}

ssize_t Socket::sendto(const std::string &buf, const std::string &path,
                       int sendto_flags) {
  return sendto(static_cast<const void *>(buf.c_str()), buf.size(),
                path.c_str(), sendto_flags);
}

ssize_t Socket::recvfrom(void *buf, size_t len, char *source, size_t source_len,
                         int recvfrom_flags) {
  if (buf == NULL) {
  ...
  bytes = ::recvfrom(sockfd_, buf, len, recvfrom_flags,
                     reinterpret_cast<struct sockaddr *>(&saddr), &socksize);

  if (source != NULL && source_len > 0) {
    memcpy(source, saddr.sun_path,
           source_len < sizeof(saddr.sun_path) ? source_len
...
ssize_t Socket::recvfrom(void *buf, size_t len, std::string &source,
                         int recvfrom_flags) {
  if (buf == NULL) {
    LOG_ERROR << "rcvfrom: Buffer is NULL!";
    return -1;
  ...
  memcpy(source_cstr.get(), saddr.sun_path,
         107 < sizeof(saddr.sun_path) ? 107 : sizeof(saddr.sun_path));
...
ssize_t Socket::recvfrom(std::string &buf, std::string &source,
                         int recvfrom_flags) {
  if (buf.empty()) {
    LOG_ERROR << "rcvfrom: Buffer is NULL!";
    return -1;
  ...
  bytes = ::recvfrom(sockfd_, cbuf.get(), buf.size(), recvfrom_flags,
                     reinterpret_cast<struct sockaddr *>(&saddr), &socksize);
                       ...
                       int sndto_flags) // flags: sendto()
{
  ...
  ssize_t bytes;
  bytes = ::sendto(sockfd_, buf, len, sndto_flags, dst.getSockAddr(),
                   dst.getSockLen());

  return bytes;
}

ssize_t Socket::sendto(const std::string &buf, const InetAddress &dst,
                       int sndto_flags) {
  return sendto(buf.c_str(), buf.size(), dst, sndto_flags);
}

ssize_t Socket::recvfrom(void *buf, size_t len, InetAddress &src,
                         int rcvfrom_flags) {
  if (-1 == sockfd_) {
    LOG_ERROR << "Socket already closed!";
  }
...
ssize_t Socket::recvfrom(std::string &buf, InetAddress &src,
                         int rcvfrom_flags) {
  ssize_t bytes;
  using std::unique_ptr;
...
void Socket::setMulticastIF(const std::string &address) {
  ...
  ::setsockopt(sockfd_, IPPROTO_IP, IP_MULTICAST_IF, &addr,
               static_cast<socklen_t>(sizeof addr));
}

void Socket::setLoopBack(bool on) {
  int optval = on ? 1 : 0;
  ::setsockopt(sockfd_, IPPROTO_IP, IP_MULTICAST_LOOP, &optval,
               static_cast<socklen_t>(sizeof optval));
  // FIXME CHECK
}

void Socket::setTTL(const unsigned char &ttl) {
  ::setsockopt(sockfd_, IPPROTO_IP, IP_MULTICAST_TTL, &ttl,
               static_cast<socklen_t>(sizeof ttl));
}

void Socket::setAddMemberShip(struct ip_mreq &mreq) {
  ::setsockopt(sockfd_, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
               static_cast<socklen_t>(sizeof mreq));
}