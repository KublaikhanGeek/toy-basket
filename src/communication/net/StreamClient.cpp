...
StreamClient::StreamClient(EventLoop* loop, const InetAddress& serverAddr, const std::string& nameArg)
    : loop_(CHECK_NOTNULL(loop))
    ...
    , nextConnId_(1)
{
    connector_->setNewConnectionCallback(std::bind(&StreamClient::newConnection, this, _1));
    // FIXME setConnectFailedCallback
    LOG_INFO << "StreamClient::StreamClient[" << name_ << "] - connector " << get_pointer(connector_);
}
...
StreamClient::~StreamClient()
{
    ...
    else
    {
        connector_->stop();
        // FIXME: HACK
        TimerManager::getInstance()->addTimer(1000, std::bind(&detail::removeConnector, connector_));
    }
}
...
{
    // FIXME: check state
    LOG_INFO << "StreamClient::connect[" << name_ << "] - connecting to " << connector_->serverAddress().toString();
    connect_ = true;
    connector_->start();
}
...
void StreamClient::newConnection(int sockfd)
{
    ...
    conn->setCloseCallback(std::bind(&StreamClient::removeConnection, this, _1)); // FIXME: unsafe
    {
        std::lock_guard<std::mutex> lock(mutex_);
        connection_ = conn;
    }
