...
void toyBasket::defaultMessageCallback(const StreamConnectionPtr &,
                                       Buffer *buf) {
  buf->retrieveAll();
}

StreamConnection::StreamConnection(EventLoop *loop, const std::string &nameArg,
  ...
  LOG_INFO << "StreamConnection::ctor[" << name_ << "] at " << this
           << " fd=" << sockfd;
  socket_->setKeepAlive(true);
}

StreamConnection::~StreamConnection() {
  LOG_INFO << "StreamConnection::dtor[" << name_ << "] at " << this
...
void StreamConnection::send(const StringPiece &message) {
  if (state_ == kConnected) {
    if (loop_->isInLoopThread()) {
      ...
      void (StreamConnection::*fp)(const StringPiece &message) =
          &StreamConnection::sendInLoop;
...
void StreamConnection::forceClose() {
  ...
  if (state_ == kConnected || state_ == kDisconnecting) {
    setState(kDisconnecting);
    loop_->queueInLoop(
        std::bind(&StreamConnection::forceCloseInLoop, shared_from_this()));
  }
}
...
void StreamConnection::handleError() {
  ...
  char t_errnobuf[512];
  LOG_ERROR << "StreamConnection::handleError [" << name_
            << "] - SO_ERROR = " << err << " "
