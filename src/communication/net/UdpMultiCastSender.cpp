...
UdpMultiCastSender::UdpMultiCastSender(const InetAddress& GroupAddr, const std::string& nameArg)
...
void UdpMultiCastSender::setLoopBack(bool on)
{
    socket_.setLoopBack(on);
}
...
{
    ssize_t ret = ::sendto(socket_.fd(), message, static_cast<unsigned long long>(len), 0, groupAddr_.getSockAddr(),
                           groupAddr_.getSockLen());
    if (ret < 0)
    {
        LOG_ERROR << name_ << " UDP MultiCastSener sendto error: " << strerror(errno);
    }
}
