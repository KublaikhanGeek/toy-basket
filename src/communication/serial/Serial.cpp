...
Serial::Serial(EventLoop* loop, const std::string& nameArg, const std::string& path)
    : loop_(loop)
    , devicePath_(path)
    , fd_(-1)
    , name_(nameArg)
...
void Serial::showPortParam()
{
    ...
    LOG_INFO << name_ << " baud rate is " << (Npt.c_cflag & CBAUD);      // band rate B115200 is 4098
    LOG_INFO << name_ << " data bit is " << (Npt.c_cflag & CSIZE);       // data bits SC8 is 48
    LOG_INFO << name_ << " stop bit is " << (Npt.c_cflag & CSTOPB);      // stop bit is 0
    LOG_INFO << name_ << " parity is " << (Npt.c_cflag & PARENB);        // no parity is 0
    LOG_INFO << name_ << " flow control is " << (Npt.c_cflag & CRTSCTS); // no flow contral is 0
}
...
void Serial::sendInLoop(const void* data, size_t len)
{
    ...
    if (!faultError && remaining > 0)
    {
        ...
        {
            loop_->queueInLoop(std::bind(highWaterMarkCallback_, this, oldLen + remaining));
        }
